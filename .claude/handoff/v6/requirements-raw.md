...
`[[pages/planning/cpp-mcp-codexgraph-gap]]` (table row "Query-side tool":
**Blocker**).
...
schema changed. The version bump is a minor (`0.4.0`) because the surface
grows. The rename-invariant test from v5 still applies — new tool names MUST
be unprefixed (`query_graphdb`, `describe_graph_schema`).
...
**Dependencies:** none upstream. US-V6-Q2 depends on the schema-discovery
mechanism this story introduces (live introspection of the backend).
...
the server what node and edge types exist (with property names and rough
counts), so that I can compose meaningful `query_graphdb` calls without
...
backend, so that the same class of regressions caught in v3 (false-positive
unit tests on a write tool that didn't actually work) cannot recur on the read
surface.
