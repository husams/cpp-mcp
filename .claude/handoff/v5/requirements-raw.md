...
**Plan reference:** `[[~/workspace/wiki/pages/planning/cpp-mcp-codexgraph-gap]]` — v5 roadmap; this story is a prerequisite naming cleanup before S1 (`cpp_query_graphdb`) and S2 (`cpp_translate_query`) land, and is itself a rename of those proposed tool names as well.
...
**Out of scope:** no behavioral changes — argument shapes, return shapes, cache semantics, error envelopes (other than tool-name text) are byte-identical to v4.
...
**Priority:** P1 — must ship in the same release as the rename, can ship in the same PR.
...
- **Test parity gate.** Pre-rename test counts must equal post-rename test counts. If a test is renamed or deleted, that is itself a deviation requiring justification.
