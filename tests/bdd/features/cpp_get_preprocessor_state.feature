Feature: cpp_get_preprocessor_state — Retrieve macro definitions and conditional state (US-6)
  ...
  Scenario: Happy path macros and conditionals returned
    When cpp_get_preprocessor_state is called on "config_macros.cpp"
  ...
  Scenario: Macro from -D flag appears with defined_at null
    When cpp_get_preprocessor_state is called on "config_macros.cpp" with flags including "-DDEBUG=1"
  ...
  Scenario: File with no macros or conditionals returns empty lists
    ...
    When cpp_get_preprocessor_state is called on "ast_test.cpp"
  ...
  Scenario: Default flags when build_path is None
    When cpp_get_preprocessor_state is called on "config_macros.cpp" with no build_path
