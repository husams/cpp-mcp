Feature: get_ast — Retrieve scoped annotated AST subtree (US-4)
  ...
  Scenario: Partial AST with parse error list for file with missing include
    ...
    When get_ast is called on "broken_partial.cpp" with format="json"
  ...
  Scenario: PATH_VIOLATION for path-traversal in file_path
    When get_ast is called with file_path "../../etc/passwd"
