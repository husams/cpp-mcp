Feature: cpp_get_header_info — Inspect include graph and exported symbols (US-5)
  ...
  Scenario: Happy path full header info returned
    When cpp_get_header_info is called on "header_api.h"
  ...
  Scenario: Header with no includes returns empty include lists
    ...
    When cpp_get_header_info is called on "header_standalone.h"
  ...
  Scenario: Unresolvable include appears in missing_includes
    ...
    When cpp_get_header_info is called on "header_missing_include.h"
  ...
  Scenario: Default flags when build_path is None
    When cpp_get_header_info is called on "header_standalone.h" with no build_path
