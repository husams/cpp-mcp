Feature: cpp_get_type_info — Retrieve type details for a symbol
  ...
  Scenario: Happy path — full type info for a concrete type
    ...
    When cpp_get_type_info is called with that file at line 5 col 5
  ...
  Scenario: Auto-typed variable resolves to concrete canonical type
    Given the file "types_test.cpp" is copied to the allowed root
    When cpp_get_type_info is called with that file at line 8 col 6
  ...
  Scenario: Template instantiation shows expanded type
    Given the file "types_test.cpp" is copied to the allowed root
    When cpp_get_type_info is called with that file at line 18 col 10
  ...
  Scenario: Incomplete type returns null size and alignment — not an error
    Given the file "types_test.cpp" is copied to the allowed root
    When cpp_get_type_info is called with that file at line 21 col 8
  ...
  Scenario: FILE_NOT_FOUND for non-existent file
    When cpp_get_type_info is called with a non-existent file at line 1 col 1
    Then the response code is "FILE_NOT_FOUND"

  @SC_US_3_7
  Scenario: INVALID_POSITION for out-of-range position
    Given the file "types_test.cpp" is copied to the allowed root
    When cpp_get_type_info is called with that file at line 0 col 0
    Then the response code is "INVALID_POSITION"

  @SC_US_3_8
  Scenario: PATH_VIOLATION for path-traversal in file_path
    When cpp_get_type_info is called with file_path "../../etc/shadow" at line 1 col 1
